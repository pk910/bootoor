...
type IPDiscovery struct {
	...
	minDistinctIPs     int                                       // Minimum distinct reporter IPs needed
...
func NewIPDiscovery(cfg IPDiscoveryConfig) *IPDiscovery {
	...
	if cfg.MinDistinctIPs <= 0 {
		cfg.MinDistinctIPs = DefaultMinDistinctIPs
	}
...
func (ipd *IPDiscovery) ReportIP(ip net.IP, port uint16, reporterID string, reporterIP net.IP) {
	...
	if reporterIP == nil {
		// Must have reporter IP
		return
	}

	// Determine if IPv4 or IPv6 (for the reported address)
	isIPv6 := ip.To4() == nil
	...
	report.reporterIPs[reporterIPStr]++

	*totalReports++

	distinctIPCount := len(report.reporterIPs)
...
// - For initial consensus: requires minimum reports, minimum distinct IPs, and majority threshold
	...
	if *consensusReached && totalRecentCount >= ipd.minReports {
		...
		if maxRecentAddr != "" && maxRecentAddr != currentAddrKey && maxRecentReport != nil {
			...
			if recentMajority >= ipd.majorityThreshold && distinctIPCount >= ipd.minDistinctIPs {
				...
				reports[maxRecentAddr] = maxRecentReport

				*currentConsensusIP = maxRecentReport.ip
				*currentConsensusPort = maxRecentReport.port
				*totalReports = maxRecentCount

				// Call callback for address change
				if ipd.onConsensusReached != nil {
					ip := maxRecentReport.ip
					port := maxRecentReport.port
...
func (ipd *IPDiscovery) GetConsensusPort() uint16 {
	ipd.mu.RLock()
	defer ipd.mu.RUnlock()
	return ipd.currentConsensusIPv4Port
}
